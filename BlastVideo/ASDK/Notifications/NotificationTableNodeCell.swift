...
class NotificationTableNodeCell: ASCellNode {
    ...
    init(post: Post, user: UserObject? = nil) {
        self.contentNode = NotificationTableNodeCellContent(post: post, user: user)
        super.init()
...
class NotificationTableNodeCellContent: ASDisplayNode {
    ...
    init(post: Post, user: UserObject?) {
        ...
        userImgNode.url = URL(string: post.photoUrl ?? "" )
        userImgNode.style.preferredSize = CGSize(width: UIScreen.screenWidth() * 0.725, height: (UIScreen.screenWidth()/post.ratio!) * 0.725)
    ...
    override func layoutSpecThatFits(_ constrainedSize: ASSizeRange) -> ASLayoutSpec {
        ...
        vertStackSpec.children = [userImgNode, usernameNode]
